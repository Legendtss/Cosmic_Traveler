...
function showPage(pageName) {
  ...
  // Trigger page pop-in animation for streaks page
  if (pageName === 'streaks') {
    const streaksShell = document.querySelector('.streaks-shell');
    if (streaksShell) {
      // Reset animation state
      streaksShell.classList.remove('active');
      requestAnimationFrame(() => {
        streaksShell.classList.add('active');
      });
    }
  }
...
const USER_STATS_KEY = 'fittrack_user_stats_v1';

function getUserStats(userId) {
  try {
    const raw = localStorage.getItem(`${USER_STATS_KEY}_${userId}`);
    const defaults = {
      totalPoints: 0,
      currentStreak: 0,
      longestStreak: 0,
...
async function updateUserProgressAfterTaskCompletion(taskId) {
  const task = taskUiState.tasks.find(t => t.id === taskId);
  if (!task) return;

  const userId = activeDemoUserId;
  const stats = getUserStats(userId);

  // 1. Calculate points for this task
  const points = POINTS_STRUCTURE.task[task.priority] || 0;

  // 2. Update daily progress
  const today = toLocalDateKey(new Date());
  if (stats.dailyProgress.date !== today) {
    stats.dailyProgress = { points: 0, date: today };
  }

  const potentialDailyPoints = stats.dailyProgress.points + points;
  const cappedDailyPoints = Math.min(potentialDailyPoints, POINTS_STRUCTURE.daily_caps.tasks);
  const pointsToAdd = cappedDailyPoints - stats.dailyProgress.points;

  stats.dailyProgress.points += pointsToAdd;
  stats.totalPoints += pointsToAdd;

  // 3. Check streak condition
  const yesterday = toLocalDateKey(new Date(Date.now() - 86400000));
  if (stats.lastUpdate === yesterday) {
    stats.currentStreak++;
  } else if (stats.lastUpdate !== today) {
...
async function refreshDashboardMetrics() {
  try {
    ...
    let workouts;
    if (activeDemoUserId) {
      const stats = getUserStats(activeDemoUserId);
      const meals = getActiveUserMealsData();
      const today = todayDateKey();
    ...
    const userStats = summary.stats || getUserStats(activeDemoUserId);
    ...
    const dashStreak = document.getElementById('dash-streak-value');
    const dashLevel = document.getElementById('dash-level-value');
    const dashPoints = document.getElementById('dash-points-value');
    const streaksCurrent = document.getElementById('streaks-current');
    const streaksLongest = document.getElementById('streaks-longest');
    const streaksPoints = document.getElementById('streaks-points');
    const streaksLevel = document.getElementById('streaks-level');

    if(sidebarStreak) sidebarStreak.textContent = `${userStats.currentStreak} Days`;
    if(sidebarLevel) sidebarLevel.textContent = `Level ${userStats.level}`;
    if(sidebarPoints) sidebarPoints.textContent = userStats.totalPoints;
    if(dashStreak) dashStreak.textContent = `${userStats.currentStreak} Days`;
    if(dashLevel) dashLevel.textContent = `Level ${userStats.level}`;
    if(dashPoints) dashPoints.textContent = `${userStats.totalPoints} Points`;
    if(streaksCurrent) streaksCurrent.textContent = `${userStats.currentStreak} Days`;
    if(streaksLongest) streaksLongest.textContent = `${userStats.longestStreak} Days`;
    if(streaksPoints) streaksPoints.textContent = userStats.totalPoints;
    if(streaksLevel) streaksLevel.textContent = `Level ${userStats.level}`;
...
function setupStreaks() {
  ...
  // Animate ring + bar + pct text from 0 on every page open
  if (ringEl) {
  ...
  requestAnimationFrame(() => {
    requestAnimationFrame(() => {
      ...
      if (levelPctEl) levelPctEl.style.opacity = '1';
    });
  });

  // Trigger glint effect on stat cards with staggered timing
  const statCards = document.querySelectorAll('.streaks-stat-card');
